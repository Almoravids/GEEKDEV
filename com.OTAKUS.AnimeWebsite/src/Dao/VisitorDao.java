...
public class VisitorDao implements IDaoIUDGGA<Visitor,String>{
	...
	public boolean update(Visitor visitor) throws DataAccessException, ClassNotFoundException, SQLException {
	    int rs=  DaoConnection.getConnection().update("update visitor set type_user=? where username=?",visitor.getType(),visitor.getUserName());
	...
	public Visitor get(String username) throws DataAccessException, ClassNotFoundException, SQLException {
		 Visitor visitor=DaoConnection.getConnection().queryForObject("select * from visitor where username='"+username+"'",new Visitor());
		 return visitor;
	}

	@Override
	public boolean insert(Visitor visitor) throws DataAccessException, ClassNotFoundException, SQLException {
		int rs=DaoConnection.getConnection().update("insert into visitor values(?,?,?,?,?,'user',?)",visitor.getUserName(),visitor.getName(),visitor.getLastName(),
    ...
    public boolean grade(String grade){return false;}
}
