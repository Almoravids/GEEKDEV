...
public class Method {
  ...
  private JdbcTemplate j=new JdbcTemplate();
...
/********************************GetANiME*******************************/
       ...
       getConnection();    
         ...
         for (Anime a:list){
        	 List<ALias> aliasList=new ArrayList<ALias>();
        		...
        		if (alias.getAnimeId()==a.getId())
        			aliasList.add(alias);
        	    	a.setAlias(aliasList);
        	a.setGenre(getGenre(a.getId()));}
  ...
  public List<ALias> getAlies()throws ClassNotFoundException, SQLException{
		getConnection();
			return (List<ALias>) j.query("select * from alias",new ALias());
					}
  
public List<ALias> getAlias(int idAnime)throws ClassNotFoundException, SQLException{
	getConnection();
		return (List<ALias>)j.query("select * from alias where id_anime="+idAnime,new ALias()); 
  }
...
public void insertAlias(Anime anime) throws ClassNotFoundException, SQLException{
	  ...
	  for(ALias alias : anime.getAlias())
		     j.update("insert into alias values(alias_seq.nextval,?,?)",alias.getAlias(),anime.getId());
  }
...
public void insertKind(Anime anime) throws ClassNotFoundException, SQLException{
	  ...
	  for(Type type : anime.getGenre())
		     j.update("insert into kind values(?,?)",anime.getId(),j.queryForInt("select id_type from type where name=?",type.getType()));
  }
...
/*********************** .::| UPDATING |::. ******************/
  public void updateAnime(Anime anime) throws ClassNotFoundException, SQLException{
	  getConnection();
	    int a=  j.update("update anime set description=? , link_img=? , id_statue=? where id_anime=?",anime.getDesc(),anime.getImageLink(),anime.getStatue(),anime.getId());
	    if (a==1) {
	    	if (!anime.getAlias().equals(getAlias(anime.getId()))){
	    deleteAlias(anime.getId());
	     insertAlias(anime);}
	    	if (!anime.getGenre().equals(getGenre(anime.getId()))){
	     deleteKind(anime.getId());
	     insertKind(anime);}
	    }
  ...
  public void deleteAlias(int idAnime) throws ClassNotFoundException, SQLException{
	  getConnection();
	  j.update("delete from alias where id_anime="+idAnime);
  }
